...
const getSavingsSplit = (amount) => [
  { name: 'Emergency Fund',      percent: 30, color: '#f59e0b' },
  { name: 'Long-term Investing', percent: 40, color: '#6366f1' },
  { name: 'Short-term Goals',    percent: 20, color: '#22c55e' },
  { name: 'Cash Reserve',        percent: 10, color: '#a78bfa' },
].map(i => ({
  ...i,
...
export default function Budget() {
  ...
  const savingsSplitData = result ? getSavingsSplit(result.invest) : [];
            ...
            <AnimatePresence>
              {showResult && (
                ...
                >
                  ...
                  <AnimatePresence mode="wait">
                    ...
                    {activeTab === 'invest' && (
                      <motion.div
                        ...
                        exit="exit"
                      >
                        <p className={styles.Budget__investIntro}>Example monthly savings allocation</p>
                        <p className={styles.Budget__investSubIntro}>
                          Monthly savings available: <strong>{fmt(result.invest)}</strong>
                        </p>

                        <div className={styles.Budget__chartBox}>
                          <ResponsiveContainer width="100%" height={180}>
                            <BarChart data={savingsSplitData} layout="vertical" margin={{ left: 0, right: 16 }}>
                        ...
                        <div className={styles.Budget__investEducateCard}>
                          <h4>Where can Long-term Investing include?</h4>
                          <p>Long-term investing may include investment options such as:</p>
                          <ul>
                            <li>Mutual Funds</li>
                            <li>ETFs</li>
                            <li>Stocks</li>
                          ...
                          <p>The choice depends on your financial goals, investment horizon and risk tolerance.</p>
                          <p className={styles.Budget__investEducateNote}>This information is educational only.</p>
                        </div>

                        <div className={styles.Budget__disclaimer}>
                          This allocation is an illustrative budgeting example intended for educational purposes only. It is not personalized financial or investment advice. Please consult a SEBI-registered investment adviser before making investment decisions.
                        </div>
                      </motion.div>
                    )}
                    ...
                    {activeTab === 'projection' && (
                      ...
                      >
                        <p className={styles.Budget__projIntro}>
                          Illustrative projection assuming a 12% annual return.
                        </p>
                        ...
                        <div className={styles.Budget__disclaimer}>
                          This projection is based on an assumed annual return of 12% for illustration only. Actual returns may vary significantly. This is not a guarantee of future performance.
                        </div>
                      </motion.div>
                    )}
                  </AnimatePresence>
                  ...
                  <div className={styles.Budget__footerDisclaimer}>
                    <p>
                      Nidhify provides budgeting tools and financial planning calculators for educational purposes only. Budget allocations, savings plans, and projections are illustrative examples and are not personalized financial or investment advice. Actual investment returns may vary. Please consult a SEBI-registered investment adviser before making investment decisions.
                    </p>
                  </div>
                </motion.div>
              )}
...
// import { useNavigate } from 'react-router-dom';
// import { motion, AnimatePresence } from 'framer-motion';
// import { ArrowLeft, Wallet, TriangleAlert, AlertCircle, CheckCircle, Home, Smartphone, Package, Lock, Heart, BarChart3, Shield, TrendingUp, Target, Sprout, Lightbulb, Calendar } from 'lucide-react';
// import {
//   PieChart, Pie, Cell, Tooltip, ResponsiveContainer,
//   BarChart, Bar, XAxis, YAxis,
//   LineChart, Line, CartesianGrid,
...
//   const breakdownRows = result ? [
//     { icon: <Lock size={18} color="#ef4444" />, iconClass: styles['Budget__bIcon--fixed'],     label: 'Fixed Costs',    val: result.fixedCosts, color: '#ef4444', sub: 'Rent + EMI + Others' },
//     { icon: <Shield size={18} color="#f59e0b" />, iconClass: styles['Budget__bIcon--emergency'], label: 'Emergency Fund', val: result.emergency,  color: '#f59e0b', sub: `Target: ${fmt(result.emergencyTarget)}` },
//     { icon: <TrendingUp size={18} color="#22c55e" />, iconClass: styles['Budget__bIcon--invest'],     label: 'Invest',         val: result.invest,     color: '#22c55e', sub: 'SIP in mutual funds' },
//     { icon: <Target size={18} color="#6366f1" />, iconClass: styles['Budget__bIcon--life'],       label: 'Life Money',     val: result.life,       color: '#6366f1', sub: 'Guilt-free spending' },
//     { icon: <Sprout size={18} color="#a78bfa" />, iconClass: styles['Budget__bIcon--growth'],     label: 'Self Growth',    val: result.selfGrowth, color: '#a78bfa', sub: 'Courses, health, skills' },
//   ] : [];
...
//                   <div className={styles.Budget__summaryRow}>
//                     {[
//                       { icon: <Lock size={20} color="#dc2626" />, wrapClass: styles['Budget__summaryIconWrap--fixed'],   value: fmt(result.fixedCosts), label: `Fixed · ${result.fixedPercent}%` },
//                       { icon: <Heart size={20} color="#059669" />, wrapClass: styles['Budget__summaryIconWrap--free'],     value: fmt(result.remaining),  label: 'Free Money' },
//                       { icon: <BarChart3 size={20} color="#6366f1" />, wrapClass: styles['Budget__summaryIconWrap--savings'],  value: `${result.savingsRate}%`, label: 'Savings Rate' },
//                     ].map((card, i) => (
//                       <motion.div
//                         key={i}
//                         className={styles.Budget__summaryCard}
...
//                           transition={{ delay: 0.4, type: 'spring', stiffness: 300, damping: 26 }}
//                         >
//                           <Lightbulb size={20} className={styles.Budget__insightIcon} />
//                           <p>{result.insight}</p>
//                         </motion.div>
...
//                         >
//                           <h3 className={styles.Budget__yearTitle}>
//                             <Calendar size={16} color="#065f46" style={{ marginRight: 4, verticalAlign: -2 }} /> If you follow this all year
//                           </h3>
//                           <div className={styles.Budget__yearGrid}>
//                             {[
//                               { value: fmt(result.invest * 12),   label: 'Invested',         color: '#22c55e' },
